...
func (data AAAAuthentication) GetEnumConstraints() []helpers.FieldEnumConstraint {
	return nil
}
...
func (data AAAAuthentication) GetStringLengthConstraints() []helpers.FieldStringLengthConstraint {
	return nil
}
...
func (data AAAAuthentication) GetPatternConstraints() []helpers.FieldPatternConstraint {
	return nil
}
...
func (data *AAAAuthentication) updateFromBody(ctx context.Context, res []byte, version string) {
	for i := range data.Login {
		...
		if value := r.Get("list-name"); value.Exists() && (value.Type == gjson.String || value.Type == gjson.Number) && !data.Login[i].List.IsNull() {
