...
func (data *RouterOSPF) updateFromBody(ctx context.Context, res []byte, version string) {
	if value := gjson.GetBytes(res, "mpls.traffic-eng.router-id.ip-address"); value.Exists() && value.Type == gjson.String && !data.MplsTrafficEngRouterIdIpv4Address.IsNull() {
	...
	if value := gjson.GetBytes(res, "mpls.traffic-eng.router-id.interface"); value.Exists() && value.Type == gjson.String && !data.MplsTrafficEngRouterIdInterfaceName.IsNull() {
	...
	if value := gjson.GetBytes(res, "mpls.traffic-eng.autoroute-exclude.route-policy"); value.Exists() && value.Type == gjson.String && !data.MplsTrafficEngAutorouteExcludeRoutePolicy.IsNull() {
	...
	if value := gjson.GetBytes(res, "segment-routing.sr-prefer.prefix-list"); value.Exists() && value.Type == gjson.String && !data.SegmentRoutingSrPreferPrefixList.IsNull() {
	...
	for i := range data.AffinityMaps {
		...
		if value := r.Get("affinity-attribute-name"); value.Exists() && value.Type == gjson.String && !data.AffinityMaps[i].AffinityName.IsNull() {
	...
	for i := range data.FlexAlgos {
		...
		for ci := range data.FlexAlgos[i].AffinityExcludeAny {
			...
			if value := cr.Get("affinity-attribute-name"); value.Exists() && value.Type == gjson.String && !data.FlexAlgos[i].AffinityExcludeAny[ci].AffinityName.IsNull() {
		...
		for ci := range data.FlexAlgos[i].AffinityIncludeAny {
			...
			if value := cr.Get("affinity-attribute-name"); value.Exists() && value.Type == gjson.String && !data.FlexAlgos[i].AffinityIncludeAny[ci].AffinityName.IsNull() {
		...
		for ci := range data.FlexAlgos[i].AffinityIncludeAll {
			...
			if value := cr.Get("affinity-attribute-name"); value.Exists() && value.Type == gjson.String && !data.FlexAlgos[i].AffinityIncludeAll[ci].AffinityName.IsNull() {
		...
		for ci := range data.FlexAlgos[i].SrlgExcludeAny {
			...
			if value := cr.Get("srlg-name"); value.Exists() && value.Type == gjson.String && !data.FlexAlgos[i].SrlgExcludeAny[ci].SrlgName.IsNull() {
	...
	if value := gjson.GetBytes(res, "monitor-convergence.prefix-list"); value.Exists() && value.Type == gjson.String && !data.MonitorConvergencePrefixList.IsNull() {
	...
	for i := range data.SrlgNames {
		...
		if value := r.Get("srlg-name"); value.Exists() && value.Type == gjson.String && !data.SrlgNames[i].SrlgName.IsNull() {
	...
	if value := gjson.GetBytes(res, "distribute.link-state.allow-prefix.route-policy"); value.Exists() && value.Type == gjson.String && !data.DistributeLinkStateAllowPrefixRoutePolicy.IsNull() {
...
func (data *RouterOSPF) fromBody(ctx context.Context, res []byte, version string) {
	if value := gjson.GetBytes(res, "mpls.traffic-eng.router-id.ip-address"); value.Exists() && value.Type == gjson.String {
		data.MplsTrafficEngRouterIdIpv4Address = types.StringValue(value.String())
	}
	if value := gjson.GetBytes(res, "mpls.traffic-eng.router-id.interface"); value.Exists() && value.Type == gjson.String {
		data.MplsTrafficEngRouterIdInterfaceName = types.StringValue(value.String())
	}
	...
	if value := gjson.GetBytes(res, "mpls.traffic-eng.autoroute-exclude.route-policy"); value.Exists() && value.Type == gjson.String {
		data.MplsTrafficEngAutorouteExcludeRoutePolicy = types.StringValue(value.String())
	}
	...
	if value := gjson.GetBytes(res, "segment-routing.sr-prefer.prefix-list"); value.Exists() && value.Type == gjson.String {
		data.SegmentRoutingSrPreferPrefixList = types.StringValue(value.String())
	}
	...
	if value := gjson.GetBytes(res, "affinity-map.affinity-attributes.affinity-attribute"); value.Exists() {
		...
		value.ForEach(func(k, v gjson.Result) bool {
			...
			if cValue := v.Get("affinity-attribute-name"); cValue.Exists() && cValue.Type == gjson.String {
		...
		value.ForEach(func(k, v gjson.Result) bool {
			...
			if cValue := v.Get("affinity.exclude-any.affinity-attributes.affinity-attribute"); cValue.Exists() {
				...
				cValue.ForEach(func(ck, cv gjson.Result) bool {
					...
					if ccValue := cv.Get("affinity-attribute-name"); ccValue.Exists() && ccValue.Type == gjson.String {
	...
	if value := gjson.GetBytes(res, "monitor-convergence.prefix-list"); value.Exists() && value.Type == gjson.String {
		data.MonitorConvergencePrefixList = types.StringValue(value.String())
	}
	...
	if value := gjson.GetBytes(res, "distribute.link-state.allow-prefix.route-policy"); value.Exists() && value.Type == gjson.String {
		data.DistributeLinkStateAllowPrefixRoutePolicy = types.StringValue(value.String())
	}
		...
		for j := range data.FlexAlgos {
			...
			if found {
				...
				if !state.FlexAlgos[i].FastRerouteDisable.IsNull() && data.FlexAlgos[j].FastRerouteDisable.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "fast-reroute/disable"))
				}
				if !state.FlexAlgos[i].AdvertiseDefinition.IsNull() && data.FlexAlgos[j].AdvertiseDefinition.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "advertise-definition"))
				}
				if !state.FlexAlgos[i].MetricTypeTeMetric.IsNull() && data.FlexAlgos[j].MetricTypeTeMetric.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "metric-type/te-metric"))
				}
				if !state.FlexAlgos[i].MetricTypeDelay.IsNull() && data.FlexAlgos[j].MetricTypeDelay.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "metric-type/delay"))
				}
				if !state.FlexAlgos[i].PrefixMetric.IsNull() && data.FlexAlgos[j].PrefixMetric.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "prefix-metric"))
				}
				if !state.FlexAlgos[i].MicroloopAvoidanceDisable.IsNull() && data.FlexAlgos[j].MicroloopAvoidanceDisable.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "microloop/avoidance/disable"))
				}
				if !state.FlexAlgos[i].Priority.IsNull() && data.FlexAlgos[j].Priority.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", state.getPath(), keyString), "priority"))
				}
				break
			}
		}
		...
		for j := range data.AffinityMaps {
			...
			if found {
				if !state.AffinityMaps[i].BitPosition.IsNull() && data.AffinityMaps[j].BitPosition.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/affinity-map/affinity-attributes/affinity-attribute%v", state.getPath(), keyString), "bit-position"))
				}
				break
			}
		}
...
func (data *RouterOSPF) getEmptyLeafsDelete(ctx context.Context, version string) []string {
	...
	for i := range data.FlexAlgos {
		...
		if !data.FlexAlgos[i].FastRerouteDisable.IsNull() && !data.FlexAlgos[i].FastRerouteDisable.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", data.getPath(), keyString), "fast-reroute/disable"))
		}
		if !data.FlexAlgos[i].AdvertiseDefinition.IsNull() && !data.FlexAlgos[i].AdvertiseDefinition.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", data.getPath(), keyString), "advertise-definition"))
		}
		if !data.FlexAlgos[i].MetricTypeTeMetric.IsNull() && !data.FlexAlgos[i].MetricTypeTeMetric.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", data.getPath(), keyString), "metric-type/te-metric"))
		}
		if !data.FlexAlgos[i].MetricTypeDelay.IsNull() && !data.FlexAlgos[i].MetricTypeDelay.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", data.getPath(), keyString), "metric-type/delay"))
		}
		if !data.FlexAlgos[i].PrefixMetric.IsNull() && !data.FlexAlgos[i].PrefixMetric.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", data.getPath(), keyString), "prefix-metric"))
		}
		if !data.FlexAlgos[i].MicroloopAvoidanceDisable.IsNull() && !data.FlexAlgos[i].MicroloopAvoidanceDisable.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/flex-algos/flex-algo%v", data.getPath(), keyString), "microloop/avoidance/disable"))
		}
	}
	for i := range data.AffinityMaps {
		keys := [...]string{"affinity-attribute-name"}
