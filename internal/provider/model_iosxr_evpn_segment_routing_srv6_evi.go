...
func (data EVPNSegmentRoutingSRv6EVI) GetStringLengthConstraints() []helpers.FieldStringLengthConstraint {
	return nil
}
...
func (data EVPNSegmentRoutingSRv6EVI) GetPatternConstraints() []helpers.FieldPatternConstraint {
	return nil
}
...
func (data *EVPNSegmentRoutingSRv6EVI) updateFromBody(ctx context.Context, res []byte, version string) {
	...
	for i := range data.Locators {
		...
		if value := r.Get("locator-name"); value.Exists() && value.Type == gjson.String && !data.Locators[i].LocatorName.IsNull() {
		...
		for j := range data.Locators {
			...
			if found {
				if !state.Locators[i].UsidAllocationWideLocalIdBlock.IsNull() && data.Locators[j].UsidAllocationWideLocalIdBlock.IsNull() {
					deletedItems = append(deletedItems, path.Join(fmt.Sprintf("%v/locators/locator%v", state.getPath(), keyString), "usid/allocation/wide-local-id-block"))
				}
				break
			}
		}
...
func (data *EVPNSegmentRoutingSRv6EVI) getEmptyLeafsDelete(ctx context.Context, version string) []string {
	...
	for i := range data.Locators {
		...
		if !data.Locators[i].UsidAllocationWideLocalIdBlock.IsNull() && !data.Locators[i].UsidAllocationWideLocalIdBlock.ValueBool() {
			emptyLeafsDelete = append(emptyLeafsDelete, path.Join(fmt.Sprintf("%v/locators/locator%v", data.getPath(), keyString), "usid/allocation/wide-local-id-block"))
		}
	}
